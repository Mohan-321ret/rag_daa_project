  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Security Score', value: '98/100', icon: <Shield className="w-4 h-4" />, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
          { label: 'Active Sessions', value: '47', icon: <Users className="w-4 h-4" />, color: 'text-blue-400', bg: 'bg-blue-500/10' },
          { label: 'Failed Logins', value: '3', icon: <AlertTriangle className="w-4 h-4" />, color: 'text-amber-400', bg: 'bg-amber-500/10' },
          { label: 'Encryption', value: 'AES-256', icon: <Lock className="w-4 h-4" />, color: 'text-violet-400', bg: 'bg-violet-500/10' },
        ].map((s, i) => (
      ...
      <div className="flex items-center gap-1 bg-white/[0.03] border border-white/[0.07] rounded-xl p-1 w-fit">
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as typeof activeTab)}
            className={`px-4 py-2 rounded-lg text-xs font-medium transition-all ${activeTab === tab.id ? 'bg-blue-600 text-white' : 'text-white/40 hover:text-white/70'}`}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <h3 className="text-sm font-semibold text-white/70 mb-4">Security Controls</h3>
          ...
          <Card>
            <h3 className="text-sm font-semibold text-white/70 mb-4">Compliance Status</h3>
            <div className="space-y-3">
              {[
                ...
                <div key={c.standard} className="flex items-center gap-3">
                  <span className="text-xs font-mono font-bold text-white/60 w-20">{c.standard}</span>
                  <div className="flex-1 h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${c.score}%` }} transition={{ duration: 0.8 }}
                      className="h-full rounded-full bg-gradient-to-r from-emerald-500 to-emerald-400" />
                  </div>
                  <span className="text-xs text-emerald-400 font-semibold w-8">{c.score}%</span>
      ...
      {activeTab === 'rbac' && (
        <Card>
          <h3 className="text-sm font-semibold text-white/70 mb-4">Permissions Matrix</h3>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  <th className="text-left py-3 px-4 text-[11px] font-semibold text-white/40 uppercase tracking-wider">Role</th>
                  {['Read', 'Write', 'Delete', 'Manage', 'Audit'].map(p => (
                    <th key={p} className="text-center py-3 px-4 text-[11px] font-semibold text-white/40 uppercase tracking-wider">{p}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {permissions.map((row, i) => (
                  <motion.tr key={row.role} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }}
                    ...
                    <td className="py-3 px-4">
                      <span className="text-xs font-semibold text-white/80">{row.role}</span>
                    </td>
                    {[row.read, row.write, row.delete, row.manage, row.audit].map((allowed, j) => (
                      <td key={j} className="py-3 px-4 text-center">
                        {allowed ? <span className="text-emerald-400 text-base">✓</span> : <span className="text-white/20 text-base">—</span>}
                      </td>
                    ))}
                  </motion.tr>
                ))}
      ...
      {activeTab === 'audit' && (
        <Card>
          <h3 className="text-sm font-semibold text-white/70 mb-4">Audit Logs</h3>
          <div className="space-y-2">
            {mockAuditLogs.map((log, i) => (
              <motion.div key={log.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                ...
                <div className={`w-2 h-2 rounded-full flex-shrink-0 ${log.status === 'success' ? 'bg-emerald-400' : log.status === 'failed' ? 'bg-red-400' : 'bg-amber-400'}`} />
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-mono font-semibold text-white/70">{log.action}</span>
                    <span className="text-[10px] text-white/30">·</span>
                    <span className="text-[11px] text-white/50">{log.resource}</span>
                  </div>
                  <p className="text-[10px] text-white/30 mt-0.5">User: {log.userId} · IP: {log.ip}</p>
                </div>
                <div className="text-right">
                  <StatusBadge status={log.status} />
                  <p className="text-[10px] text-white/25 mt-1">{formatDateTime(log.timestamp)}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {activeTab === 'sessions' && (
        <Card>
          <h3 className="text-sm font-semibold text-white/70 mb-4">Active Sessions</h3>
          <div className="space-y-3">
            {mockUsers.map((user, i) => (
              <motion.div key={user.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                className="flex items-center gap-4 p-3 rounded-xl bg-white/[0.02] border border-white/[0.05]">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] text-white/40">{user.email} · {user.department}</p>
                </div>
                <span className="text-[10px] bg-white/[0.06] text-white/50 px-2 py-0.5 rounded-full capitalize">{user.role}</span>
                <StatusBadge status="active" />
