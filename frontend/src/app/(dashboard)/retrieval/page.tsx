...
export default function RetrievalPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <div className="flex gap-3 mb-4">
          ...
          <div className="flex items-center gap-2 bg-white/[0.04] border border-white/[0.08] rounded-xl px-3">
            <span className="text-xs text-white/40">Top-K:</span>
            <select value={topK} onChange={e => setTopK(Number(e.target.value))} className="bg-transparent text-xs text-white/70 outline-none">
              {[3, 5, 10, 20].map(k => <option key={k} value={k} className="bg-[#12121f]">{k}</option>)}
            </select>
          </div>
          <button onClick={handleRetrieve} disabled={!query.trim() || loading} className="flex items-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-blue-600 to-violet-600 text-sm font-semibold text-white hover:from-blue-500 hover:to-violet-500 transition-all disabled:opacity-50">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {methods.map(m => (
            <div key={m.id}
              className={`p-3 rounded-xl border text-left transition-all ${activeRoute === m.id ? 'border-blue-500/40 bg-blue-500/10' : 'border-white/[0.06] bg-white/[0.02]'}`}>
              <div className={`inline-flex p-1.5 rounded-lg bg-gradient-to-br ${m.color} mb-2`}>
      ...
      {result && (
        <Card>
          <h3 className="text-xs font-semibold text-white/50 uppercase tracking-wider mb-3">Router Decision</h3>
          <div className="flex items-center gap-3 flex-wrap mb-2">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-medium ${methodColors[result.route.route] ?? 'bg-white/[0.04] text-white/40'}`}>
              <ChevronRight className="w-3 h-3" />
              <span className="uppercase font-bold">{result.route.route}</span>
            </div>
            <span className="text-xs text-white/50">{result.route.reason}</span>
          </div>
          {result.route.signals.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-2">
              {result.route.signals.map((s, i) => (
                <span key={i} className="text-[10px] bg-white/[0.06] text-white/50 px-2 py-0.5 rounded-full">{s}</span>
              ))}
            </div>
          )}
        </Card>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-white/70">{result ? `${result.results.length} Retrieved Chunks` : 'Results'}</h3>
          {elapsed !== null && (
        ...
        <AnimatePresence>
          {loading ? (
            ...
            <motion.div key={`${r.document_id}-${r.chunk_index}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
              className="p-4 rounded-2xl bg-white/[0.03] border border-white/[0.07] hover:border-white/[0.12] transition-all">
              <div className="flex items-start justify-between gap-4 mb-3">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-[11px] font-bold text-white/30">#{i + 1}</span>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium uppercase ${methodColors[r.source] ?? 'text-white/50 bg-white/[0.06]'}`}>{r.source}</span>
                  <span className="text-[11px] text-white/50 truncate max-w-[200px]">{r.document_id} · chunk {r.chunk_index}</span>
                </div>
              ...
              <p className="text-xs text-white/60 leading-relaxed">{r.text_preview}</p>
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
