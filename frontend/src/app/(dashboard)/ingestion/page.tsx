  ...
  const columns = [
    { key: 'original_filename', label: 'Document', sortable: true, render: (_: unknown, row: DocRow) => (
      <div className="flex items-center gap-2">
        ...
        <div>
          <p className="text-xs font-medium text-white/80 max-w-[240px] truncate">{row.original_filename}</p>
          <p className="text-[10px] text-white/30">{row.document_id}</p>
        </div>
      </div>
    )},
    { key: 'document_type', label: 'Type', render: (v: unknown) => <span className="text-[11px] uppercase font-mono text-white/50">{String(v)}</span> },
    ...
    { key: 'visibility', label: 'Visibility', render: (v: unknown) => (
      <span className="text-[10px] uppercase font-mono px-1.5 py-0.5 rounded bg-white/[0.06] text-white/50">{String(v ?? 'domain')}</span>
    )},
    { key: 'word_count', label: 'Words', sortable: true, render: (v: unknown) => <span className="text-xs text-white/60">{Number(v).toLocaleString()}</span> },
    { key: 'ocr_used', label: 'OCR', render: (v: unknown) => <span className="text-[11px] text-white/40">{v ? 'Yes' : 'No'}</span> },
    { key: 'upload_date', label: 'Uploaded', sortable: true, render: (v: unknown) => <span className="text-[11px] text-white/40">{formatDateTime(String(v))}</span> },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'Total Uploaded', value: total, icon: <FileText className="w-4 h-4" />, color: 'text-blue-400' },
          { label: 'Processing', value: processingCount, icon: <Clock className="w-4 h-4" />, color: 'text-amber-400' },
          { label: 'Completed', value: completedCount, icon: <CheckCircle className="w-4 h-4" />, color: 'text-emerald-400' },
          { label: 'Failed', value: failedCount, icon: <AlertCircle className="w-4 h-4" />, color: 'text-red-400' },
        ].map((s, i) => (
      ...
      <Card>
        <Can
          ...
          fallback={
            <div className="border-2 border-dashed border-white/[0.08] rounded-xl p-10 text-center">
              <Upload className="w-10 h-10 text-white/10 mx-auto mb-3" />
              <p className="text-sm font-medium text-white/40">Your role cannot upload documents</p>
              <p className="text-xs text-white/25 mt-1">Ask a Domain Manager or Analyst to ingest this on your behalf.</p>
            </div>
          }
        >
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div>
                <label className="text-[11px] font-medium text-white/50 mb-1 block">Visibility</label>
                <select value={visibility} onChange={e => setVisibility(e.target.value as typeof visibility)}
                  ...
                  <option value="domain">Domain (same-domain members)</option>
                  <option value="restricted">Restricted (explicit grants)</option>
                </select>
              </div>
              <div>
                <label className="text-[11px] font-medium text-white/50 mb-1 block">
                  Domain {needsDomain && <span className="text-red-400">*</span>}
                </label>
                <select value={domainId} onChange={e => setDomainId(e.target.value)} disabled={visibility === 'global'}
                  className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2 text-xs text-white outline-none focus:border-blue-500/50 disabled:opacity-40">
                  <option value="">{visibility === 'global' ? 'Not required' : 'Select a domain…'}</option>
                  {selectableDomains.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                </select>
              </div>
              <div>
                <label className="text-[11px] font-medium text-white/50 mb-1 block">Department</label>
              ...
              <div>
                <label className="text-[11px] font-medium text-white/50 mb-1 block">Permissions</label>
            ...
            {visibility === 'restricted' && (
              <div>
                <label className="text-[11px] font-medium text-white/50 mb-1.5 flex items-center gap-1.5"><Lock className="w-3 h-3" /> Grant access to</label>
                <div className="flex flex-wrap gap-1.5 p-2.5 bg-white/[0.02] border border-white/[0.06] rounded-xl max-h-28 overflow-y-auto">
                  {allUsers.length === 0 && <span className="text-[11px] text-white/25">Loading users…</span>}
                  {allUsers.map(u => (
                    <button key={u.id} type="button"
                      onClick={() => setRestrictedUserIds(prev => prev.includes(u.id) ? prev.filter(x => x !== u.id) : [...prev, u.id])}
                      className={`px-2.5 py-1 rounded-full text-[11px] font-medium border transition-all ${restrictedUserIds.includes(u.id) ? 'bg-blue-600 border-blue-500 text-white' : 'bg-white/[0.04] border-white/[0.08] text-white/50 hover:text-white/80'}`}>
                      {u.full_name || u.email}
                    </button>
                  ))}
                </div>
            ...
            {needsDomain && !domainId && (
              <p className="text-[11px] text-amber-400">Select a domain before uploading — required unless visibility is Global.</p>
            )}

            <div {...getRootProps()} className={`border-2 border-dashed rounded-xl p-10 text-center transition-all ${!canUpload ? 'opacity-50 cursor-not-allowed border-white/[0.06]' : 'cursor-pointer ' + (isDragActive ? 'border-blue-500 bg-blue-500/10' : 'border-white/[0.08] hover:border-white/20 hover:bg-white/[0.02]')}`}>
              <input {...getInputProps()} />
              <motion.div animate={isDragActive ? { scale: 1.05 } : { scale: 1 }}>
                <Upload className="w-10 h-10 text-white/20 mx-auto mb-3" />
                <p className="text-sm font-medium text-white/60">{isDragActive ? 'Drop files here...' : 'Drag & drop files or click to browse'}</p>
                <p className="text-xs text-white/30 mt-1">Supports PDF, DOCX, HTML, TXT, PPTX</p>
                <div className="flex items-center justify-center gap-2 mt-4">
                  {['PDF', 'DOCX', 'HTML', 'TXT', 'PPTX'].map(t => (
                    <span key={t} className="text-[10px] bg-white/[0.06] text-white/40 px-2 py-1 rounded-lg font-mono">{t}</span>
                  ))}
                </div>
              </motion.div>
            </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {u.status === 'error' && <span className="text-[10px] text-red-400" title={u.error}><AlertCircle className="w-3.5 h-3.5 inline" /> {u.error}</span>}
                        {u.status === 'cancelled' && <span className="text-[10px] text-white/40">Cancelled</span>}
                        {u.status === 'uploading' && <span className="text-[10px] text-blue-400">{Math.round(u.progress)}%</span>}
                        {u.status === 'queued' && <span className="text-[10px] text-amber-400">Queued…</span>}
                        {u.status === 'processing' && (
                          <span className="text-[10px] text-blue-400">{u.currentStage ? (stageLabels[u.currentStage] ?? u.currentStage) : 'Processing…'}</span>
                        )}
                        <button onClick={() => setUploads(p => p.filter(x => x.id !== u.id))} className="text-white/20 hover:text-white/50"><X className="w-3 h-3" /></button>
                      </div>
                    </div>
                    <div className="h-1 bg-white/[0.06] rounded-full overflow-hidden">
                      <motion.div
                        ...
                        className={`h-full rounded-full ${
                          ...
                          : u.status === 'cancelled' ? 'bg-white/20'
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-1.5">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search documents..." className="bg-transparent text-xs text-white/60 placeholder:text-white/25 outline-none w-40" />
            </div>
            <div className="flex items-center gap-1 bg-white/[0.04] border border-white/[0.08] rounded-xl p-1">
              {['all', 'completed', 'processing', 'failed'].map(f => (
                <button key={f} onClick={() => setFilter(f)}
                  className={`px-3 py-1 rounded-lg text-[11px] font-medium capitalize transition-all ${filter === f ? 'bg-blue-600 text-white' : 'text-white/40 hover:text-white/70'}`}>
                  {f}
                </button>
              ))}
            </div>
