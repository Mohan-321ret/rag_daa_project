...
export default function VerificationPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <div className="flex gap-3">
          <textarea value={answer} onChange={e => setAnswer(e.target.value)} rows={3}
            ...
            className="flex-1 bg-gray-50 dark:bg-white/[0.04] border border-gray-200 dark:border-white/[0.08] rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-white/25 outline-none focus:border-blue-500/50 transition-all resize-none" />
      ...
      {error && <Card><p className="text-sm text-red-600 dark:text-red-400 text-center py-2">{error}</p></Card>}

      {loading && (
        <Card>
          <div className="flex items-center justify-center py-12">
            <div className="flex items-center gap-3 text-gray-500 dark:text-white/50">
              <ShieldCheck className="w-5 h-5 animate-pulse text-blue-500 dark:text-blue-400" />
      ...
      {result && !loading && (
        <>
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            className={`p-5 rounded-2xl border ${trust >= 0.8 ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200 dark:border-emerald-500/20' : trust >= 0.6 ? 'bg-amber-50 dark:bg-amber-500/10 border-amber-200 dark:border-amber-500/20' : 'bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-500/20'}`}>
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <p className={`text-3xl font-bold ${trust >= 0.8 ? 'text-emerald-600 dark:text-emerald-400' : trust >= 0.6 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'}`}>{(trust * 100).toFixed(0)}%</p>
                {result.was_rewritten && <p className="text-xs text-amber-600 dark:text-amber-400/70">Answer was corrected</p>}
              </div>
            </div>
            <div className="mt-3 h-2 bg-gray-200 dark:bg-white/[0.06] rounded-full overflow-hidden">
          ...
          {result.was_rewritten && (
            <Card>
              <h3 className="text-xs font-semibold text-gray-500 dark:text-white/50 uppercase tracking-wider mb-2">Final (Corrected) Answer</h3>
              <p className="text-sm text-gray-800 dark:text-white/70 leading-relaxed bg-gray-50 dark:bg-white/[0.03] rounded-xl p-4 border border-gray-200 dark:border-white/[0.05]">{result.final_answer}</p>
            </Card>
          )}
              ...
              <div className="flex items-center gap-1 bg-gray-100 dark:bg-white/[0.03] border border-gray-200 dark:border-white/[0.07] rounded-xl p-1">
                {(['all', 'supported', 'contradicted'] as const).map(f => (
                  <button key={f} onClick={() => setFilter(f)}
                    className={`flex-1 py-1.5 rounded-lg text-[11px] font-medium capitalize transition-all ${filter === f ? 'bg-blue-600 text-white' : 'text-gray-500 dark:text-white/40 hover:text-gray-900 dark:hover:text-white/70'}`}>
                    {f}
                  </button>
                ))}
              </div>
              {filtered.length === 0 && <p className="text-xs text-gray-400 dark:text-white/30 text-center py-6">No claims in this category.</p>}
              {filtered.map((claim, i) => (
                <motion.button key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                  onClick={() => setSelected(claim)}
                  className={`w-full p-3 rounded-xl border text-left transition-all ${selected === claim ? 'border-blue-500/30 bg-blue-50 dark:bg-blue-500/10' : 'border-gray-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] hover:border-gray-300 dark:hover:border-white/[0.12]'}`}>
                  <div className="flex items-start gap-2">
                    {statusIcon(claim.status)}
                    <div className="flex-1">
                      <p className="text-[11px] text-gray-800 dark:text-white/70 leading-tight font-medium">{claim.claim}</p>
                      <p className="text-[10px] text-gray-400 dark:text-white/30 mt-1">similarity: {(claim.statement_similarity * 100).toFixed(0)}%</p>
                    </div>
                  </div>
                </motion.button>
              ))}
              ...
              {selected ? (
                <Card>
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium capitalize ${selected.status === 'contradicted' ? 'bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-400' : selected.status === 'supported' ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400' : 'bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400'}`}>
                        {selected.status === 'contradicted' ? '⚠️ Contradicted' : selected.status === 'supported' ? '✓ Supported' : '? Unverifiable'}
                      </span>
                      <p className="text-sm font-semibold text-gray-900 dark:text-white mt-2">{selected.claim}</p>
                    </div>
                  </div>

                  <div className="mb-4">
                    <p className="text-xs text-gray-500 dark:text-white/40 mb-2">Statement Similarity</p>
                    <div className="flex items-center gap-3">
                      <div className="flex-1 h-3 bg-gray-200 dark:bg-white/[0.06] rounded-full overflow-hidden">
                      ...
                      <span className="text-lg font-bold text-gray-800 dark:text-white/80">{(selected.statement_similarity * 100).toFixed(0)}%</span>
                    </div>
                  </div>

                  {selected.status === 'contradicted' && (
                    <div className="p-3 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 mb-4">
                      <div className="flex items-center gap-2 mb-1">
                        <AlertTriangle className="w-3.5 h-3.5 text-red-600 dark:text-red-400" />
                        <span className="text-xs font-semibold text-red-600 dark:text-red-400">Contradiction Detected</span>
                      </div>
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-xs font-semibold text-gray-500 dark:text-white/50">Evidence</p>
                    </div>
                    {selected.evidence_text ? (
                      <div className="flex items-start gap-2 p-2.5 rounded-xl bg-gray-50 dark:bg-white/[0.03] border border-gray-200 dark:border-white/[0.05]">
                        <span className="text-[10px] bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-400 px-1.5 py-0.5 rounded font-mono">{selected.evidence_document_id}</span>
                        <p className="text-xs text-gray-700 dark:text-white/60">{selected.evidence_text}</p>
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-500 dark:text-emerald-400 ml-auto flex-shrink-0" />
                      </div>
                    ) : (
                      <div className="p-3 rounded-xl bg-red-50 dark:bg-red-500/5 border border-red-200 dark:border-red-500/15 text-xs text-red-600 dark:text-red-400/70">
