  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span className="text-[11px] text-white/30 ml-auto">{total} user{total === 1 ? '' : 's'}</span>
      ...
      {loading ? (
        ...
        <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl overflow-hidden overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-white/[0.06]">
                {['Name', 'Role', 'Status', 'Domains', 'Department', 'Last Login', ''].map(h => (
                  <th key={h} className="px-4 py-3 text-[11px] font-semibold text-white/40 uppercase tracking-wider whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {users.map((u, i) => (
                <motion.tr key={u.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: Math.min(i * 0.02, 0.3) }}
                  ...
                  <td className="px-4 py-3">
                    <p className="text-xs font-medium text-white/80">{u.full_name || u.email.split('@')[0]}</p>
                    <p className="text-[11px] text-white/35">{u.email}</p>
                  </td>
                  <td className="px-4 py-3"><RoleBadge role={u.role} /></td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium border ${u.status === 'active' ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/25' : 'bg-gray-500/15 text-gray-400 border-gray-500/25'}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1 max-w-[180px]">
                      {u.domains.length === 0
                        ...
                        : u.domains.map(d => <span key={d.id} className="text-[10px] bg-white/[0.06] text-white/50 px-1.5 py-0.5 rounded-full">{d.name}</span>)}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-xs text-white/50">{u.department || '—'}</td>
                  <td className="px-4 py-3 text-[11px] text-white/40">{u.last_login ? formatDateTime(u.last_login) : 'Never'}</td>
                  <td className="px-4 py-3">
      ...
      <div className="space-y-4">
        ...
        {mode === 'create' && (
          <div>
            ...
            <input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="name@company.com"
              className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/20 outline-none focus:border-blue-500/50" />
          </div>
        )}
        {mode === 'create' && (
          <div>
            <label className="text-xs font-medium text-white/60 mb-1.5 block">Initial password</label>
        ...
        <div>
          ...
          <select value={role} onChange={e => setRole(e.target.value)} disabled={!canEditRole}
            className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2 text-sm text-white outline-none focus:border-blue-500/50 disabled:opacity-50">
            {selectableRoles.map(r => <option key={r} value={r}>{ROLE_LABELS[r]}</option>)}
            {mode === 'edit' && user && !selectableRoles.includes(user.role as Role) && (
              <option value={user.role}>{ROLE_LABELS[user.role as Role] ?? user.role} (current)</option>
            )}
          </select>
          {!canEditRole && <p className="text-[11px] text-white/25 mt-1">You don&apos;t have permission to change roles.</p>}
        </div>

        <div>
          <label className="text-xs font-medium text-white/60 mb-1.5 block">Domains</label>
          <div className="flex flex-wrap gap-1.5 p-2.5 bg-white/[0.02] border border-white/[0.06] rounded-xl min-h-[44px]">
            {selectableDomains.length === 0 && <span className="text-[11px] text-white/25">No domains available to assign.</span>}
            {selectableDomains.map(d => (
              <button key={d.id} type="button" disabled={!canEditDomains} onClick={() => toggleDomain(d.id)}
                className={`px-2.5 py-1 rounded-full text-[11px] font-medium border transition-all disabled:opacity-50 disabled:cursor-not-allowed ${domainIds.includes(d.id) ? 'bg-blue-600 border-blue-500 text-white' : 'bg-white/[0.04] border-white/[0.08] text-white/50 hover:text-white/80'}`}>
                {d.name}
              </button>
            ))}
          </div>
          ...
          {domains.map(d => (
            <motion.div key={d.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex items-start justify-between mb-2">
                <div>
                  <p className="text-sm font-semibold text-white/85">{d.name}</p>
                  <p className="text-[11px] font-mono text-white/30">{d.key}</p>
                </div>
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium border ${d.is_active ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/25' : 'bg-gray-500/15 text-gray-400 border-gray-500/25'}`}>
                  {d.is_active ? 'Active' : 'Inactive'}
                </span>
              </div>
              <p className="text-xs text-white/40 mb-3 min-h-[2rem]">{d.description || 'No description.'}</p>
              <Can permission={Permission.DOMAIN_MANAGE}>
  ...
  return (
    <Modal open={open} onClose={onClose} title={domain ? `Edit ${domain.name}` : 'Create Domain'} maxWidth="max-w-sm">
      <div className="space-y-4">
        ...
        {!domain && (
          <div>
            <label className="text-xs font-medium text-white/60 mb-1.5 block">Key (URL-safe, permanent)</label>
        ...
        <div>
          ...
          <textarea value={description} onChange={e => setDescription(e.target.value)} rows={2}
            className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/20 outline-none focus:border-blue-500/50 resize-none" />
        </div>
        {domain && (
