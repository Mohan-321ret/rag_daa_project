...
function ExpandedTrace({ log }: { log: any }) {
  return (
    ...
    >
      <div className="px-4 py-4 border-t border-white/[0.05] bg-white/[0.01] grid grid-cols-2 gap-4">
        <div>
          <p className="text-[10px] text-white/30 uppercase tracking-wider mb-2">Query</p>
          <p className="text-xs text-white/70 leading-relaxed">{log.query_text}</p>
        </div>
        <div>
          <p className="text-[10px] text-white/30 uppercase tracking-wider mb-2">Answer</p>
          <p className="text-xs text-white/60 leading-relaxed line-clamp-4">{log.answer_text || '—'}</p>
        </div>
        <div className="col-span-2 grid grid-cols-4 gap-3">
          {[
            ...
            <div key={item.label} className="bg-white/[0.03] rounded-lg p-2.5">
              ...
              <p className={`text-xs text-white/65 ${item.mono ? 'font-mono text-[10px]' : ''}`}>{String(item.value)}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="space-y-1">
            {filtered.map(log => {
              ...
              return (
                <div key={log.query_id} className="bg-white/[0.02] border border-white/[0.06] rounded-xl overflow-hidden hover:border-white/10 transition-colors">
                  <button className="w-full flex items-center gap-4 px-4 py-3 text-left" onClick={() => setExpandedId(isOpen ? null : log.query_id)}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-white/75 truncate">{log.query_text}</p>
                      <div className="flex items-center gap-3 mt-1">
                        <span className={`px-1.5 py-0.5 rounded text-[10px] border ${ROUTE_COLORS[log.route ?? ''] ?? 'bg-white/[0.06] text-white/40 border-white/10'}`}>{log.route}</span>
                        {log.intent && <span className="text-[10px] text-white/30">{log.intent}</span>}
                        <span className="text-[10px] text-white/25">{formatDateTime(log.created_at)}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 flex-shrink-0">
                      {conf !== undefined && conf !== null && (
                        <span className={`text-xs font-mono font-semibold ${conf >= 0.7 ? 'text-emerald-400' : conf >= 0.45 ? 'text-amber-400' : 'text-red-400'}`}>
                          {Math.round(conf * 100)}%
                        </span>
                      )}
                      <span className="text-[11px] text-white/25">{log.latency_ms ? `${Math.round(log.latency_ms)}ms` : ''}</span>
                      {isOpen ? <ChevronUp className="w-4 h-4 text-white/30" /> : <ChevronDown className="w-4 h-4 text-white/20" />}
                    </div>
                  </button>
                  <AnimatePresence>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-10 text-sm text-white/30">No traces found</div>
            )}
          </div>
        )}
      </AdminSectionShell>
