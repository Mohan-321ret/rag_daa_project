...
export default function RepositoryPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {storageData.map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            ...
            <p className="text-xl font-bold text-white mb-1">{s.used} <span className="text-sm font-normal text-white/30">/ {s.total} GB</span></p>
            <div className="h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
              <motion.div initial={{ width: 0 }} animate={{ width: `${(s.used / s.total) * 100}%` }} transition={{ duration: 0.8, delay: i * 0.1 }}
                className="h-full rounded-full" style={{ background: s.color }} />
            </div>
            <p className="text-[10px] text-white/30 mt-1">{((s.used / s.total) * 100).toFixed(0)}% used</p>
          </motion.div>
        ))}
      </div>
        ...
        <Card>
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-white/40 mt-0.5">847,293 vectors · text-embedding-3-large · 1536 dimensions</p>
            </div>
            ...
            {filtered.slice(0, 8).map((doc, i) => (
              <motion.div key={doc.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-white/30">{doc.department} · {doc.language.toUpperCase()}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-semibold text-white/70">{doc.embeddingCount > 0 ? doc.embeddingCount.toLocaleString() : '—'}</p>
                  <p className="text-[10px] text-white/30">vectors</p>
                </div>
                <div className="w-24">
                  {doc.confidenceScore > 0 ? <ConfidenceMeter value={doc.confidenceScore} size="sm" /> : <span className="text-[10px] text-white/20">Processing...</span>}
                </div>
      ...
      {activeTab === 'graph' && (
        <Card>
          <h3 className="text-sm font-semibold text-white mb-4">Knowledge Graph Nodes</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {mockKnowledgeNodes.map((node, i) => (
              <motion.div key={node.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }}
                className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:border-white/[0.12] transition-all">
                <div className={`w-3 h-3 rounded-full flex-shrink-0 ${node.type === 'concept' ? 'bg-blue-400' : node.type === 'entity' ? 'bg-violet-400' : node.type === 'document' ? 'bg-emerald-400' : 'bg-amber-400'}`} />
                <div className="flex-1">
                  <p className="text-xs font-medium text-white/80">{node.label}</p>
                  <p className="text-[10px] text-white/30 capitalize">{node.type} · {node.connections} connections</p>
                </div>
                <div className="w-20">
          ...
          <div className="mt-4 flex items-center gap-4 text-[11px] text-white/40">
            {[{ color: 'bg-blue-400', label: 'Concept' }, { color: 'bg-violet-400', label: 'Entity' }, { color: 'bg-emerald-400', label: 'Document' }, { color: 'bg-amber-400', label: 'Relation' }].map(l => (
            ...
            {mockKnowledgeSources.map((src, i) => (
              <motion.div key={src.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-white/40">{src.department} · Last sync: {formatDateTime(src.lastSync)}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-white">{formatNumber(src.documentCount)}</p>
      ...
      {activeTab === 'versions' && (
        <Card>
          <h3 className="text-sm font-semibold text-white mb-4">Version Repository</h3>
          <div className="space-y-2">
            {[
              ...
              <motion.div key={v.v} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="flex-1">
                  <p className="text-xs font-bold text-white">Version {v.v}</p>
                  <p className="text-[10px] text-white/40">{v.date} · {v.docs.toLocaleString()} documents</p>
                </div>
                <p className="text-xs text-white/50">{v.size}</p>
