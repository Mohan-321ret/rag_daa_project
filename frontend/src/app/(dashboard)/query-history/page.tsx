  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Queries', value: total, color: 'text-blue-400' },
          { label: 'Page', value: `${Math.floor(filters.skip / filters.limit) + 1}`, color: 'text-violet-400' },
          { label: 'Verified', value: logs.filter(q => q.verification_result === 'verified').length, color: 'text-emerald-400' },
          { label: 'Issues', value: logs.filter(q => q.hallucinations_detected > 0).length, color: 'text-red-400' },
        ].map((s, i) => (
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2 bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2 flex-1">
          ...
          <input 
            ...
            className="bg-transparent text-xs text-white/60 placeholder:text-white/25 outline-none flex-1" 
          />
        </div>
        <button
        ...
        <button
          ...
          className="px-4 py-2 bg-white/[0.05] border border-white/[0.07] hover:bg-white/[0.08] text-white rounded-xl text-xs font-medium transition-colors flex items-center gap-2"
        >
      ...
      {showAdvancedFilters && (
        ...
        >
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <label className="text-xs text-white/40 mb-2 block">Intent</label>
              <select
                value={filters.intent}
                onChange={e => setFilters(f => ({ ...f, intent: e.target.value }))}
                className="w-full bg-white/[0.05] border border-white/[0.07] rounded-lg px-2 py-1.5 text-xs text-white outline-none"
              >
            ...
            <div>
              <label className="text-xs text-white/40 mb-2 block">Retrieval Route</label>
              <select
                value={filters.route}
            ...
            <div>
              <label className="text-xs text-white/40 mb-2 block">Verification</label>
              <select
            ...
            <div>
              <label className="text-xs text-white/40 mb-2 block">Min Confidence</label>
              <input
                type="number"
                min="0"
                max="1"
            ...
            <div>
              <label className="text-xs text-white/40 mb-2 block">Max Confidence</label>
              <input
                type="number"
                min="0"
                max="1"
            ...
            <div>
              <label className="text-xs text-white/40 mb-2 block">Ticket Status</label>
              <select
            ...
            <div>
              <label className="text-xs text-white/40 mb-2 block">Retrieval Strategy</label>
              <select
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button
              ...
              className="px-4 py-2 bg-white/[0.05] border border-white/[0.07] hover:bg-white/[0.08] text-white rounded-lg text-xs font-medium transition-colors"
            >
              Clear All
            </button>
          </div>
      ...
      <div className="space-y-2">
        {loading ? (
          <div className="text-center py-8 text-white/40">Loading query logs...</div>
        ) : logs.length === 0 ? (
          <div className="text-center py-8 text-white/40">No query logs found</div>
        ) : (
          logs.map((log, i) => (
            <motion.div key={log.query_id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}
              ...
              <Link href={`/query-logs/${log.query_id}`}>
                <button className="w-full flex items-center gap-4 p-4 text-left hover:bg-white/[0.02] transition-colors"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <p className="text-sm text-white/80 truncate">{log.query_text}</p>
                    </div>
                    <div className="flex items-center gap-3 text-[11px] text-white/40 flex-wrap">
                      ...
                      {log.intent && (
                        <>
                          <span>·</span>
                          <span className="capitalize">{log.intent}</span>
                        </>
                      )}
                      {log.route && (
                        <>
                          <span>·</span>
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${methodColors[log.route as keyof typeof methodColors]}`}>
                            {log.route}
                          </span>
                        </>
                      )}
                      {log.verification_result && (
                        <>
                          <span>·</span>
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${verificationColors[log.verification_result as keyof typeof verificationColors]}`}>
                            {log.verification_result}
                          </span>
                        </>
                      )}
                  ...
                  <div className="flex items-center gap-4 flex-shrink-0">
                    <div className="text-right">
                      <p className="text-xs font-semibold text-white/70">{log.latency_ms ? formatLatency(log.latency_ms) : 'N/A'}</p>
                    ...
                    {expanded === log.query_id ? <ChevronUp className="w-4 h-4 text-white/30" /> : <ChevronDown className="w-4 h-4 text-white/30" />}
                  </div>
                </button>
              </Link>

              {expanded === log.query_id && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="border-t border-white/[0.06] p-4 bg-white/[0.01]">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                      <p className="text-[10px] text-white/30 mb-1">Complexity</p>
                      <p className="text-xs font-semibold text-white/70 capitalize">{log.complexity || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-white/30 mb-1">Retrieved Chunks</p>
                      <p className="text-xs font-semibold text-white/70">{log.retrieved_chunks}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-white/30 mb-1">Citations</p>
                      <p className="text-xs font-semibold text-white/70">{log.citation_count}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-white/30 mb-1">Grounded</p>
                      <p className={`text-xs font-semibold ${log.is_grounded ? 'text-emerald-400' : 'text-gray-400'}`}>
      ...
      {total > filters.limit && (
        <div className="flex items-center justify-between pt-4">
          <button
            ...
            className="px-4 py-2 bg-white/[0.05] border border-white/[0.07] rounded-lg text-xs font-medium text-white disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white/[0.08] transition-colors"
          >
            Previous
          </button>
